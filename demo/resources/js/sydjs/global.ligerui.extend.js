var global_ligerui_extend = {
     ...
     */
	  ...
	  setDisabled: function (inputObjName) {
		  var l = arguments.length;
		  if (l === 0) return;
		  for (var i = 0; i < l; i++) {
			  if ($("#"+arguments[i]).hasClass("Wdate")) {
				  $("#"+arguments[i]).removeClass().addClass('input-read').attr('readonly', 'readonly').removeAttr('onclick');
				  continue;
			  }
			  $("#"+arguments[i]).ligerGetComboBoxManager().setDisabled();
		  }
	  },
	  
	  setEnabled: function (inputObjName) {
		  var l = arguments.length;
		  if (l === 0) return;
		  for (var i = 0; i < l; i++) {
			  $("#"+arguments[i]).ligerGetComboBoxManager().setEnabled();
		  }
	  },
