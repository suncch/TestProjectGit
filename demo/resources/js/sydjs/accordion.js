...
$(function() {
	...
	var createMenu = function(data){
		//将当前用户权限信息保存到全局js变量中.clone一份.
		currentUserRole = jQuery.extend(true, {}, data);
	...
	var fillContent = function(data) {
		...
		$.each(dataJson, function(index, obj) {
			...
			if (obj.childrenTree) {
				...
				treeIndex += 1;
				_temp_childrenTree = obj.childrenTree;		//二级菜单
				$.each(_temp_childrenTree, function(index, tempObj) {
					tempObj.children = null;	//屏蔽三级菜单
				});
				
//				treeArr.push(obj.childrenTree);
				treeArr.push(_temp_childrenTree);
			}
			htmlStr += '</div>';
		});
